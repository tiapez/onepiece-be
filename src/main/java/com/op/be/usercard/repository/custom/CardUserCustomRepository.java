...
public interface CardUserCustomRepository extends JpaRepository<Card, Long> {
	...
	@Query("SELECT c,uc,cd "
			...
			+ "ORDER BY  c.number ")
	List<Object[]> findUserCardClassicBySet(@Param("set") String set, @Param("user") String user);
	...
	List<Object[]> findUserCardDetailsBySet(@Param("set") String set, @Param("user") String user);
